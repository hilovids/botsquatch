...
const { hasSundayPassedSince, isValidGridSpace, distanceBetweenSpaces, dredgeResult } = require("../../utility/seachartUtility.js");
...
function updateInfo(connection, discordUser){
    return new Promise((resolve, reject)=>{
        ...
        const updateQuery = `
            INSERT INTO camp_hilo (user_id, seachart_dredge)
            VALUES (${discordUser.id}, ${time})
            ON DUPLICATE KEY UPDATE 
            seachart_dredge = "${time}";
        `;
...
module.exports = {
	...
	async execute(interaction) {
        ...
        if(!hasSundayPassedSince(userData.seachart_dredge)){
            const exampleEmbed = new EmbedBuilder()
            .setColor(0x003280)
        ...
        .setURL('https://hilovids.github.io/camp-hilo/index.html')
        .setDescription(`${dredgeResult(seachartSpace) ? "HIT! DM Production to know what you found..." : "...miss... ...nothing found..."}`)
        .setThumbnail('https://imgur.com/mfc6IFp.png')
        .setTimestamp()
        await interaction.reply({ embeds: [exampleEmbed] });
        connection.end();
