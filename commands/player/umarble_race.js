...
function buildStageLeaderboardEmbed(stage, final) {
    ...
    return applyStyle(new EmbedBuilder()
        ...
        .addFields(
            { name: 'Top 5', value: top.length ? top.map(fmt).join('\n') : 'N/A', inline: false }
...
async function runRaceInThread(thread, raceResult) {
    ...
    for (const stage of stageList) {
        ...
        await thread.send({ embeds: [buildStageLeaderboardEmbed(stage, final)], files: [iconFile()] });
        await sleep(1200); // Add delay after leaderboard embed
    }
